...
# Further modified by Adam Diehl (adadiehl@umich.edu)
...
sub novo2sam {
    my %opts = ();
    getopts("ph:", \%opts);
    die("Usage: novo2sam.pl [-p, -h <header file>] <aln.novo>\n") if (@ARGV == 0);
    my $is_paired = defined($opts{p});
    # v0.1.4 -- Add sam header from file -- AD
    if ($opts{h}) {
	my $header;
	open my $IN, '<', $opts{h} || die "Cannot read header file: $!\n";
	while (<$IN>) {
	    print $_;
	}
	close $IN;
  ...
  # core loop
	...
	my $x = &novo2sam_aux($_, $s_curr, $is_paired);
	# v0.1.4 -- Check for successful return before proceeding (suppresses
	# uninitialized value errors) -- AD
	next if ($x);
...
#    print STDERR "$t[4]\n";
    ...
    @$s = ();
    # v0.1.4: Now returns 1 if unsuccessful, 0 if successful -- AD
    return 1 if ($t[4] ne 'U');
    my $len = length($t[2]);
  # read name
    $s->[0] = substr($t[0], 1);
  $s->[0] =~ s/\/[12]$//g;
